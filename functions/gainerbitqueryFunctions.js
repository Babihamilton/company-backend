...
async function getBitqueryData(query, variables) {
  ...
  //   "variables": "{}"
  // });
 // console.log(query,"query_toptrade")
  var data = JSON.stringify({
    query,
    variables,
  });
...
export async function gainergetPairCurrencies(chain, pair){
 ...
 * since ---> yyyy-mm-dd
 */
export async function gainergetChartData(
  chain,
  pair,
  baseCurrency,
  quoteCurrency,
...
export async function gainergetTokenInfo(chain, address) {
...
export async function gainergetDexTrades(chain, address) {
...
export async function gainergetPairs(chain, address) {
...
export async function gainergetNewListedPairs(chain) {
...
// export const gainergetTopTrades = async (chain) =>
// //   getBitqueryData(
...
// //   );
//   getBitqueryData(
//     `{ ethereum(network: ${chain}) { dexTrades( options: {desc: [\"tradeAmount\", \"trades\"], limit: 50} time: {since: \"${new Date(new Date().getTime() - 86400000).toISOString()}\"} ) { token: smartContract { address { address } } exchange { fullName } trades: count tradeAmount(in: USD) any(of: price) baseCurrency { address symbol name } } } }`,
//     "{}"
//   );
export async function gainergetTopTrades(chain){
  const query =  `{ ethereum(network: ${chain}) { dexTrades( options: {desc: [\"tradeAmount\", \"trades\"], limit: 68} time: {since: \"${new Date(new Date().getTime() - 86400000).toISOString()}\"} ) { token: smartContract { address { address } } exchange { fullName } trades: count tradeAmount(in: USD) any(of: price) baseCurrency { address symbol name } } } }`
  const variables = "{}";
 //  getBitqueryData(
 //     `{ ethereum(network: ${chain}) { dexTrades( options: {desc: [\"tradeAmount\", \"trades\"], limit: 100} time: {since: \"2022-09-16T06:16:18.416Z\"} ) { token: smartContract { address { address } } exchange { fullName } trades: count tradeAmount(in: USD) any(of: price) baseCurrency { address symbol name } } } }`,
 //     "{}"
 //   );
 //console.log(query,"query_testt")
 let data = await getBitqueryData(query, variables);
 return JSON.stringify(data);
   }
export const gainergetTokenBalances = async (chain, address) => getBitqueryData(
  `{\n  ethereum(network: ${chain}) {\n    address(address: {is: \"${address}\"}) {\n      balances {\n        value\n        currency {\n          address\n          symbol\n          tokenType\n          tokenId\n        }\n      }\n    }\n  }\n}\n`,
  "{}"
)

export const gainergetTokenCreated = (chain, address) => getBitqueryData(
  `{\n  ethereum(network: ${chain}) {\n    transactions(txSender: {is: \"${address}\"}) {\n      creates {\n        smartContract {\n          currency {\n            name\n            symbol\n            tokenType\n          }\n          contractType\n        }\n        address\n      }\n    }\n  }\n}\n`,
  "{}"
)

export const gainergetOwnershipTrasnferred = (chain, addresses) => getBitqueryData(
  `{\n  ethereum(network: ethereum) {\n    smartContractEvents(\n      smartContractAddress: {in: ${addresses}}\n      smartContractEvent: {is: \"OwnershipTransferred\"}\n    ) {\n      arguments {\n        argument\n        argumentType\n        index\n        value\n      }\n      smartContract {\n        address {\n          address\n        }\n      }\n    }\n  }\n}\n`,
  "{}"
)

export async function  gainergetPriceVariation (chain, addresses) {
  const query = `{
    ethereum(network: ${chain}) {
      ${addresses.map((value, index) => `
        variation${index}: dexTrades(options: { desc: \"timeInterval.hour\"} baseCurrency: {is: \"${value.baseCurrency}\"}quoteCurrency: {is: \"${value.quoteCurrency}\"}time: {since: \"${new Date(new Date().getTime() - 86400000).toISOString()}\"}) { quotePrice timeInterval {hour(count: 24)}}`).join("")}}}`
...
// export const gainergetPriceVariation = (chain, addresses) => {
//   const query = `{
//     ethereum(network: ${chain}) {
//       ${addresses.map((value, index) => `
//         variation${index}: dexTrades(
//           options: { desc: "quotePrice", limit: 10}
...
//         ) {
//           quotePrice
//           baseCurrency {decimals symbol}
//           quoteCurrency {decimals symbol}
...
export const gainerget24HourVolume = (chain, pair) => getBitqueryData(
...
export const gainergetCirculatedSupply = (chain, currency) => getBitqueryData(
